...
def convert_mesh_to_sign_distance_map(mesh, name="SignedDistances", img_info=None):
    ...
    if img_info is None:
        ...
        min_bound -= extend * extra / 2.0
        img_info["spacing"] = extend * (1.0 + extra) / np.array(size)
...
def crop_to_structure(img, seg, seg_ids, relax_vox=5):
    ...
    for seg_id in seg_ids:
        ...
        max_x, max_y, max_z = (
            min(np.max(ids_x) + relax_vox, x - 1),
            min(np.max(ids_y) + relax_vox, y - 1),
            min(np.max(ids_z) + relax_vox, z - 1),
        )
        min_x, min_y, min_z = (
            max(np.min(ids_x) - relax_vox, 0),
            max(np.min(ids_y) - relax_vox, 0),
            max(np.min(ids_z) - relax_vox, 0),
        )
    ...
    return (
        img[min_x_a:max_x_a, min_y_a:max_y_a, min_z_a:max_z_a],
        seg[min_x_a:max_x_a, min_y_a:max_y_a, min_z_a:max_z_a],
    )
...
def create(seg_fn, hs_size, ds_size, ref_fn=None, r_ids=[1]):
    ...
    img_info = {
        "spacing": seg_ds.GetSpacing(),
        "min_bound": seg_ds_vtk.GetOrigin(),
        "size": seg_ds_vtk.GetDimensions(),
    }
    ...
    for i in r_ids:  # Myo only
        ...
        if sdf is None:
            ...
            sdf.GetPointData().AddArray(
                sdf_i.GetPointData().GetArray("sdf_{}".format(i))
            )
        x, y, z = sdf.GetDimensions()
        py_sdf_i = (
            vtk_to_numpy(sdf.GetPointData().GetArray("sdf_{}".format(i)))
            .reshape(z, y, x)
            .transpose(2, 1, 0)
            .astype(np.float32)
        )
...
def create_from_segmentation(seg_fn, hs_size, ds_size, ref_fn=None, r_ids=[1]):
    ...
    for i in r_ids:  # Myo only
        ...
        mesh_i = smooth_polydata(
            vtk_marching_cube(seg_i_vtk, 0, 1), 20, smoothingFactor=0.5
        )
        mesh_i = decimation(mesh_i, 0.9)
        print("MESH: ", i, mesh_i.GetNumberOfPoints())
        region_ids = np.append(
            region_ids, np.ones(mesh_i.GetNumberOfPoints()) * (i - 1)
        )
...
def process_img(img, ref, ds_size):
    ...
    py_img = (
        vtk_to_numpy(img_ds_vtk.GetPointData().GetScalars())
        .reshape(z, y, x)
        .transpose(2, 1, 0)
        .astype(np.float32)
    )
    img_ds_vtk.GetPointData().SetScalars(
        numpy_to_vtk(py_img.transpose(2, 1, 0).flatten())
    )
...
def check_image_mesh_alignment(img, mesh):
    ...
    py_img = (
        vtk_to_numpy(img.GetPointData().GetScalars())
        .reshape(z, y, x)
        .transpose(2, 1, 0)
    )
    coords = vtk_to_numpy(mesh.GetPoints().GetData())
    ...
    py_img[coords[:, 0], coords[:, 1], coords[:, 2]] = 1000.0
        ...
        for xy_sdf, y_coord in zip(y_list, y_coord_list):
            ...
            for xyz_sdf, z_coord in zip(z_list, z_coord_list):
                ...
                if np.sum(xyz_sdf < 0) > 100:
                    out_dict[str(count)] = {
                        "sdf_chunk": xyz_sdf,
                        "chunk_coord": np.array([x_i, y_i, z_i]),
                        "total_size": sdf.shape,
                    }
                    count += 1
    return out_dict
...
if __name__ == "__main__":
    ...
    seg_dir = "data/motion_seg/patient1"
    out_dir = "data/motion_seg/patient1_processed"
    ...
    for seg_fn in seg_fns_scatter:
        ...
        mesh, mesh_ls, sdf_v, sdf_v_py = create_from_segmentation(
            seg_fn, (512, 512, 512), (128, 128, 128), ref_fn, r_ids
        )
        if not mesh:
            continue
        write_vtk_polydata(mesh, os.path.join(out_dir, "vtk", "{}.vtp".format(name)))
        pickle.dump(
            sdf_v_py.astype(np.float32),
            open(os.path.join(out_dir, "pytorch", "{}.pkl".format(name)), "wb"),
        )
        if img_dir:
            img_v, img_v_py = process_img(
                sitk.ReadImage(img_fn), sitk.ReadImage(seg_fn), (128, 128, 128)
            )
            write_vtk_image(sdf_v, os.path.join(out_dir, "vtk", "{}.vti".format(name)))
            pickle.dump(
                img_v_py.astype(np.float32),
                open(os.path.join(out_dir, "pytorch_img", "{}.pkl".format(name)), "wb"),
            )
