...
def write_sampled_point(points, point_values, fn, flip=True):
    pts_py = (np.squeeze(points.detach().cpu().numpy()) + 1.0) / 2.0
    if flip:
...
def write_sdf_to_vtk_mesh(sdf, fn, thresh, decimate=0.0, keep_largest=False):
    mesh_list = []
    region_ids = []
    for i in range(sdf.shape[-1]):
        img = vtk.vtkImageData()
        ...
        img.GetPointData().SetScalars(
            numpy_to_vtk(sdf[:, :, :, i].transpose(2, 1, 0).flatten())
        )
        if keep_largest:
